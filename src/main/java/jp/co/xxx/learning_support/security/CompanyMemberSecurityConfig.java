package jp.co.xxx.learning_support.security;
...
@Order(2)
public class CompanyMemberSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.formLogin() // ログインに関する設定
			.loginPage("/company/toCompanyLoginPage") // ログイン画面に遷移させるパス(ログイン認証が必要なパスを指定してかつログインされていないとこのパスに遷移される)
			.loginProcessingUrl("/login") // ログインボタンを押した際に遷移させるパス(ここに遷移させれば自動的にログインが行われる)
			.failureUrl("/toCompanyLoginPageError") //ログイン失敗に遷移させるパス
			.defaultSuccessUrl("/company/showCompanyTrainingList", true) // 第1引数:デフォルトでログイン成功時に遷移させるパス
		...
		http.logout() // ログアウトに関する設定
			...
			.logoutSuccessUrl("/toCompanyLoginPage") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
